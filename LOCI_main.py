...
# Indicates in which files data has to be saved
...
class Base():
    ...
    def read_EEPROM(self):
        # If live then gather data from sensor, if not then use provided data
        if LIVE:
            ...
            for i in range(0, len(read), 2):
                self._dataEE[int(i/2)] = (struct.unpack('<H', read[i:i+2])[0])
            self._dataEE = self._dataEE.astype(int)
            ## Cleaning the buffers to make sure there is nothing in it
            self.ser.reset_output_buffer()
            self.ser.reset_input_buffer()
        else:
            self._dataEE_Par = np.array([166, 27039, 0, 8289, 4, 800, 992, 2828, 11342, 4486, 1165, 0
...
, 4178, 5136, 64528, 8402])
            self._dataEE = np.array([166, 27039, 0, 8289, 4, 800, 992, 2828, 11342, 4486, 1165, 0
, 6401, 0, 0, 48691, 17168, 65460, 4626, 514, 257, 61937, 57569, 49360
, 4644, 276, 57587, 53474, 49363, 49362, 45507, 37314, 31142, 12378, 56762, 4607
, 13090, 13107, 290, 56831, 56780, 4094, 8465, 12834, 8755, 4386, 65280, 52718
, 6066, 12225, 9558, 41609, 17203, 467, 28010, 26725, 9059, 127, 5046, 839
...
, 4178, 5136, 64528, 8402])
    ...
    def calculate_PixTemp(self):
        # If live then calculate temp based on real-time sensor data, if not then calculate temp based on provided data
        if LIVE:
